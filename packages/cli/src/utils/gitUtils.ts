...
export const getLatestGitHubRelease = async (
  ...
  try {
    ...
    const endpoint = `https://api.github.com/repos/google-github-actions/run-agentic-cli/releases/latest`;
    ...
    console.debug(`Failed to determine latest run-agentic-cli release:`, _error);
    throw new Error(
      `Unable to determine the latest run-agentic-cli release on GitHub.`,
    );
  }
};
