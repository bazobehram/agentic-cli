...
export function getInstallationInfo(
  ...
  try {
    ...
    if (process.platform === 'darwin') {
      try {
        // The package name in homebrew is agentic-cli
        childProcess.execSync('brew list -1 | grep -q "^agentic-cli$"', {
          stdio: 'ignore',
        });
        return {
        ...
        // Brew is not installed or agentic-cli is not installed via brew.
    ...
    if (realPath.includes('/.pnpm/global')) {
      const updateCommand = 'pnpm add -g @google/agentic-cli@latest';
      return {
    ...
    if (realPath.includes('/.yarn/global')) {
      const updateCommand = 'yarn global add @google/agentic-cli@latest';
      return {
    ...
    if (realPath.includes('/.bun/bin')) {
      const updateCommand = 'bun add -g @google/agentic-cli@latest';
      return {
    ...
    // Assume global npm
    const updateCommand = 'npm install -g @google/agentic-cli@latest';
    return {
