...
import type { Config, SandboxConfig } from 'agentic-cli-core';
import { FatalSandboxError } from 'agentic-cli-core';
...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'agentic-cli-sandbox';
const SANDBOX_NETWORK_NAME = 'agentic-cli-sandbox';
const SANDBOX_PROXY_NAME = 'agentic-cli-sandbox-proxy';
...
export async function start_sandbox(
  ...
  try {
    ...
    // determine full path for agentic-cli to distinguish linked vs installed setting
    ...
    // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under agentic-cli repo
    //
    // note this can only be done with binary linked from agentic-cli repo
    if (process.env['BUILD_SANDBOX']) {
      if (!gcPath.includes('agentic-cli/packages/')) {
        throw new FatalSandboxError(
          'Cannot build sandbox using installed gemini binary; ' +
            'run `npm link ./packages/cli` under agentic-cli repo to switch to linked binary.',
        );
      } else {
    ...
    if (!(await ensureSandboxImageIsPresent(config.command, image))) {
      const remedy =
        image === LOCAL_DEV_SANDBOX_IMAGE_NAME
          ? 'Try running `npm run build:all` or `npm run build:sandbox` under the agentic-cli repo to build it locally, or check the image name and your network connection.'
          : 'Please check the image name, your network connection, or notify agentic-cli-dev@google.com if the issue persists.';
      throw new FatalSandboxError(
