  ...
  try {
    ...
    if (isNightly) {
      ...
      if (bestUpdate && semver.gt(bestUpdate.latest, currentVersion)) {
        const message = `A new version of agentic-cli is available! ${currentVersion} → ${bestUpdate.latest}`;
        return {
          message,
      ...
      if (updateInfo && semver.gt(updateInfo.latest, currentVersion)) {
        const message = `agentic-cli update available! ${currentVersion} → ${updateInfo.latest}`;
        return {
          message,
